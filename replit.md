...
- **Workspace Management**: `server/workspace/` handles the lifecycle of generated applications. It creates isolated directories, writes generated files, patches hardcoded ports to use `process.env.PORT`, resolves `npm start` commands, allocates dynamic ports (4000-4099), installs dependencies, starts applications, and proxies requests. It also manages workspace states (writing-files, installing, starting, running, failed) and ensures previous workspace apps are stopped when new runs begin.
