...
Big session — reachability fixes, a real browser tool, full Google Workspace, and a new skill type for external repos.

- **Chat retry + friendlier errors** (`24b17ec`) — `streamWithRetry` wraps `client.messages.stream()` with backoff (1s/3s/7s) on overloaded / rate-limited / 5xx / 529, emits `tool_status` during retries. Outer catch translates Anthropic error types to human-readable messages so raw JSON no longer leaks into the chat stream.
- **Nav no longer detonates chat** (`1b94b89`) — Settings / Dashboard / Assets used to unmount `PromptColumn` when a project was open, losing in-flight streams. They now overlay via `.nav-overlay` with `position: absolute; z-index: 10`, keeping the workspace mounted underneath.
- **Topbar project name is a back button** (`2136cfd`) — clicking the project name in the top bar returns to the projects list. Removes the previous "how do I get out of this project" dead-end.
- **Skills actually reachable** (`38e936b`) — added `list_skills` and `read_skill` tools. The previous `skillContext` request-body field was never populated by the client, so Frank had no awareness any skill existed. He can now call `list_skills` when Brian references a skill by name and `read_skill` to load authoritative instructions. System prompt adds a `## SKILLS` section that tells Frank not to improvise if a named skill isn't registered.
- **`browser_use` tool** (`c971e64` + `150c996`, deduped in `df5d252`) — real cloud browser control via Browser Use Cloud's v3 REST (`POST /api/v3/sessions`, poll `GET /api/v3/sessions/:id`). Parallel-session race condition landed two `browser_use` definitions at once, Anthropic rejected the request with "Tool names must be unique." Kept the better of the two (uses real API fields `lastStepSummary` / `liveUrl` / `recordingUrls` / `totalCostUsd`, records real cost + token counts in `forge_usage`). Frank picks browser_use over fetch_url for SPAs, auth walls, multi-step flows, and visual verification.
- **Full Google Workspace access** (`eea7bb0`) — single-user OAuth with `GOOGLE_REFRESH_TOKEN` in the secrets vault, access tokens minted on demand and cached 55 minutes, 401-retry-once. Six new modules under `server/integrations/google/`: `auth`, `gmail`, `calendar`, `drive`, `docs`, `contacts`. 22 new Frank tools across Gmail (search, read, labels, draft, send), Calendar (list, create, update, delete), Drive (search, read, write, folder, move, delete — defaults to trash), Docs (read-as-markdown, append, replace, create), Contacts (search). 13 are approval-gated — writes, sends, and deletes. System prompt gets a `## GOOGLE WORKSPACE` section with usage guidance: prefer drafts over sends, default `sendUpdates='none'`, default `trash=true`, work under a `/Documents` folder to match Google Drive for Desktop sync. Bring-up: `GOOGLE_CLIENT_ID` + `GOOGLE_CLIENT_SECRET` in vault, redirect URI `https://forge-os.ai/api/google/auth/callback` on the OAuth app, then one trip through `/api/google/auth/start`. Tested end-to-end — Frank appended "Test" to a Drive doc on first real try.
- **Twin Claude resolved** — the "ghost" commits earlier in the session (`33c857f`, `e513c93`, `150c996`) were the Anthropic Claude Code GitHub App, triggered by prior issue/PR mentions, not a breach. Brian disabled the App. Branch protection ruleset "Claude PR" active on all branches with bypass for Repository admin + Claude App + Viktor AI Coworker (Xirider) — target kept permissive per Brian's preference. The three twin commits stayed in history; all three were legitimate improvements.
- **Repo Access Protocol** (`645733d`) — specialized skill type (`skill_type: 'repo_access'`) that lets Frank work on external repos with per-repo PATs. Schema migration added `skill_type`, `repo_owner`, `repo_name`, `repo_branch`, `repo_token_secret_key` columns to `skills` with `ADD COLUMN IF NOT EXISTS` (zero impact on existing rows). PATs stored in the vault under `REPO_TOKEN_<skill_id>`, never in the skills table or client responses, cascade-deleted with the skill. `github_*` tools refactored: `GITHUB_REPO` → `DEFAULT_GITHUB_REPO`; new `resolveRepo(ctx)` + `githubHeaders(ctx)` take a per-turn `repoContext`. Main-block narrowed from "never write main" to "never write main of ForgeOS itself" — external repos can write main freely (that's their dev per Brian's workflow; ForgeOS keeps its self-surgery prevention because Frank once started performing open-heart surgery on himself). Render auto-provision also scoped to ForgeOS's own apps/*. Chat handler scans conversation history + current message for `/<slug>` tokens, matches against computed skill slugs, builds the `repoContext` for the turn. Sticky across turns — slash once in turn 1, stays active for the conversation. Settings UI: type dropdown + conditional repo fields (owner, name, branch, password-type PAT input); edit mode leaves the PAT blank to keep existing, paste new to rotate. Read-only view shows a blue "Repo Access Protocol" badge with repo/branch/token-status metadata.
- **`forge_import` tool** (`184b2b4`) — narrow tool per Frank's spec for the Forge Intelligence brain-loop. Three params (`content_id`, `raw_text`, `edit_reason`); endpoint (`https://forgeintelligence.ai/api/content/import`), brand id (`dd482396-...`), auth header format (`X-Api-Key`), and vault key (`FORGE_API_KEY`) all encoded server-side — invisible to Frank so there's nothing he can get wrong. Error taxonomy per his spec: 401 notify-Brian, 5xx no-retry, 200 with `editMode:false` pass-through so his skill can investigate. Approval-gated alongside `gmail_send` / `drive_write_file`; every external-state write goes through Brian's consent.
- **Approval flow: timeout ≠ cancel** (`cf104c4`) — Frank reported a cancelled `forge_import` that Brian never cancelled. Root cause: 5-minute timeout on the approval promise resolved to the same `false` value as an explicit user cancel, so Frank got `"User cancelled this action."` indistinguishably from either a real cancel or an SSE drop or a slow review. Fix: promise now resolves to `'approved' | 'cancelled' | 'approve_all' | 'timeout'`; timeout tool_result explicitly says "This is NOT a user cancellation — report to Brian and ask whether to retry." Timeout bumped from 5 minutes to 30 minutes for careful review of large payloads. Server logs both sides of the approval dance (`[forge] approval_required id=…`, `[forge] approve endpoint id=… approved=…`, `[forge] approval resolved id=… outcome=…`) so future "what happened" audits are grep-able in Render logs. After deploy, Frank re-fired `forge_import` and Forge returned `editMode: true` — brain-loop closed end-to-end on its first real run.
