...
app.post('/call-complete', function(req, res) {
  ...
  var transcript = history.map(function(turn) {
    if (turn.role === 'user') {
      return 'Caller: ' + turn.content;
    }
    // Assistant turns contain raw JSON — extract the reply field for readability
    var replyText = turn.content;
    try {
      var stripped = replyText;
      if (stripped.includes('```')) {
        stripped = stripped.replace(/^```(?:json)?\s*/m, '').replace(/\s*```$/m, '');
      }
      var firstBrace = stripped.search(/[{[]/);
  ...
  // Send email
  sendEmailSummary(callerName, callerPhone, reason, notes, startTime, transcript).catch(function(err) {
...
function sendEmailSummary(name, phone, reason, notes, timestamp, transcript) {
