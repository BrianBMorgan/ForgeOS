...
app.get('/api/submissions/export', async function (req, res) {
  try {
    ...
    var submissions = await sql`
      SELECT s.id, s.title, s.bu, s.track, s.format, (SELECT full_name FROM speakers sp WHERE sp.id = s.speaker_id) as speaker_name, s.status, s.ai_score
      FROM submissions s
  ...
  try {
    ...
    var rows = await sql`
      SELECT
        ...
        (SELECT full_name FROM speakers sp WHERE sp.id = s.speaker_id) as speaker_name
      FROM submissions s
